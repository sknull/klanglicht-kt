...
class HybridScene(
    ...
    override fun fade(
        ...
        if (fadeDuration > 0) {
            runBlocking {
                ...
                fadeShellyDevice(other, fadeDuration)
                ...
                while (factor <= 1.0) {
                    factor = fadeStep(
                        otherParameterSets = otherParameterSets,
                        parameterSets = parameterSets,
                        factor = factor,
                        dmx = stage.devices.dmx,
                        otherXledFrames = otherXledFrames,
                        xledFrames = xledFrames,
                        step = step,
                        dmxFrameTime = dmxFrameTime
                    )
                }
            }
        }
    ...
    private suspend fun fadeStep(
        otherParameterSets: Map<String, ParameterSet>,
        parameterSets: Map<String, ParameterSet>,
        factor: Double,
        dmx: Dmx,
        otherXledFrames: Map<String, XledFrameDmxFadeable>,
        xledFrames: Map<String, XledFrameDmxFadeable>,
        step: Double,
        dmxFrameTime: Long
    ): Double {
        var factor1 = factor
        if (otherParameterSets.isNotEmpty()) { // only write to dmx interface if needed
            ...
            otherParameterSets.forEach { (id, otherParameterSet) ->
                ...
                if (parameterSet != null && otherParameterSet.toRgbColor() != parameterSet.toRgbColor()) {
                    val faded = parameterSet.fade(otherParameterSet, factor1, BlendMode.AVERAGE)
                    dmx.setDmxData(
        ...
        if (otherXledFrames.isNotEmpty()) {
            otherXledFrames.forEach { (id, otherXledFrame) ->
                ...
                if (xledFrame != null) {
                    val faded = xledFrame.fade(otherXledFrame, factor1, BlendMode.AVERAGE)
                    faded.write(true)
                }
            }
        }
    ...
    private suspend fun fadeShellyDevice(other: HybridScene, fadeDuration: Long) {
        coroutineScope {
