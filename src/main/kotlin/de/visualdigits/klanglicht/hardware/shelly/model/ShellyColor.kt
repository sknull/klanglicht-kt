...
class ShellyColor(
    ...
    override fun write(write: Boolean, transitionDuration: Long) {
        if (write) {
            ...
            URI("http://$ipAddress/color/0?turn=${if (getTurnOn() == true) "on" else "off"}&red=${color.red}&green=${color.green}&blue=${color.blue}&white=0&gain=${(100 * deviceGain).toInt()}&transition=$transitionDuration&")
                .toURL()
